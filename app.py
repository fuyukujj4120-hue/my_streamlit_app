...
def get_sheet_record(annotator_name: str, video_name: str):
    resp = requests.get(
        SHEET_WEBHOOK_URL,
        params={
            "secret": SHEET_SECRET,
...
def get_sheet_annotator_records(annotator_name: str):
    resp = requests.get(
        SHEET_WEBHOOK_URL,
        params={
            "secret": SHEET_SECRET,
            "action": "get_annotator_records",
            "annotator_name": annotator_name,
        },
        timeout=20,
    )
...
def get_sheet_completed_count(annotator_name: str):
    rows = get_sheet_annotator_records(annotator_name)
    return len(rows)
...
with st.sidebar:
    ...
    if annotator_name:
        try:
            st.session_state.completed = get_sheet_completed_count(annotator_name)
        except Exception:
            pass
    else:
...
if st.session_state.page == "instruction":
    ...
    if len(st.session_state.videos) == 0:
        st.warning("目前找不到影片。請先確認 VIDEOS 清單。")
    ...
    if len(st.session_state.videos) == 0:
        st.error("沒有可標註的影片，請先確認 VIDEOS 清單。")
        st.stop()

    if st.session_state.current_index >= len(st.session_state.videos):
        st.success("所有影片都標註完成了，資料已儲存到 Google Sheet。")
        st.stop()
    ...
    if annotator_name:
        try:
            saved_record = get_sheet_record(annotator_name, current_video_name)
    ...
    if saved_record and st.session_state.annotation_step == 1:
        st.info("這支影片你已經標過。你可以修改後重新儲存，系統會覆蓋 Google Sheet 舊資料。")
    ...
    if st.session_state.annotation_step == 1:
        ...
        if save_clicked:
            ...
            try:
                ...
                st.session_state.completed = get_sheet_completed_count(annotator_name)
                st.success("已儲存本筆標註到 Google Sheet。")
            except Exception as e:
                st.error(f"同步 Google Sheet 失敗：{e}")
